...
for BLOCKSIZE in "${BLOCKSIZES[@]}"; do
    for MODEL in "${MODELS[@]}"; do
        # 配置日志文件名
        LOG_FILE="$LOG_DIR/${MODEL}-${METHOD}-quant"
        ...
        python3 run_arb.py "$MODEL_DIR/$MODEL" "$DATASET" $METHOD --blocksize "$BLOCKSIZE" --salient_metric hessian --device "$DEVICE" --num_p 1 --order2_group --save #>> "${LOG_FILE}.log" 2>&1
