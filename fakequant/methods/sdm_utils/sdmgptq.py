...
import numpy as np
import sys
sys.path.append("./fakequant/methods/sdm_utils")
...
class sdmgptq_quant:
    def __init__(self, layer,hadamard=True,pb_mask=False,metric="magnitude"):
        ...
        self.pbmask=pb_mask
        #assert self.hadamard != self.pbmask
    ...
    def fasterquant(self,
        ...
        for blocki, col_st in enumerate(range(0, self.columns, blocksize)):
            ...
            if self.hadamard:
                #print(scale_config)
                Q1=sdm_quant(W1@Had,OSR=OSR,first2=first2,scale_config=scale_config)@Had.T
            elif self.pbmask:
                raise NotImplementedError("pbmask not implemented now")
                mask=pb_mask(W1,Hinv1,salient_metric=self.metric,low_frac=0.99)
                Q1=sdm_quant(W1*mask,OSR=OSR)+W1*(~mask)
                Q1+=(~mask)*W1
            else:
                Q1=sdm_quant(W1,OSR=OSR,first2=first2,scale_config=scale_config)
            W[:, col_st:col_ed] = Q1
            Err1=(W1-Q1)/torch.diag(Hinv1)
            if not disable_gptq:
                #raise NotImplementedError("GPTQ not implemented now")
