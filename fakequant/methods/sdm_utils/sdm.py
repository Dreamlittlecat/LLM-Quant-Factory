...
def extend(weight,OSR):
    return resample(weight, weight.shape[-1]*OSR,axis=-1) #return F.interpolate(weight, scale_factor=config.OSR, mode='linear')

class SigmaDeltaQuantizer:
    def __init__(self, first2=False, sd_scale=1, order=1,percentile_scale=0.95,percentile_cut=0.55):
        ...
        # print(f"sdm quantizer: first2={self.first2}, sd_scale={self.sd_scale}, order={self.order}, percentile_scale={self.percentile_scale}, percentile_cut={self.percentile_cut}")
        # raise NotImplementedError("sdm quantizer not implemented now")
    def quantize(self, input_matrix):
        ...
        max_vals = torch.quantile(torch.abs(input_matrix), self.percentile_scale, dim=-1, keepdim=True)#max_vals = torch.max(torch.abs(input_matrix), dim=-1, keepdim=True).values
        scale = max_vals * self.sd_scale
        input_matrix = input_matrix /scale
        cut_threshold_first3=torch.quantile(torch.abs(input_matrix), self.percentile_cut, dim=1)#cut_threshold_first3=torch.quantile(torch.abs(input_matrix), 55 / 100.0)
   
        for j in range(cols):
            # 计算积分器输入
            integrator_input = input_matrix[:, j] - self.previous_output
            if self.order == 1:# 一阶 Sigma-Delta 调制
                ...
                if not self.first2:  # 一阶三元量化
                    ...
                    output_matrix[:, j][self.integrator1 <=0] = -1
            elif self.order == 2:# 二阶 Sigma-Delta 调制
                self.integrator1 += integrator_input
                self.integrator2 += self.integrator1 - self.previous_output
                if not self.first2:  # 二阶三元量化
                    output_matrix[:, j][self.integrator2 >= cut_threshold_first3] = 1
                    output_matrix[:, j][self.integrator2 <= -cut_threshold_first3] = -1
                else:  # 二阶二元量化
        ...
        # 反归一化
        output_matrix = output_matrix *scale+mean_vals 
        return output_matrix

def sdm_quant(weight,OSR=2,first2=False,scale_config=None):
    ...
    if scale_config is not None:
        sd_scale=scale_config["scale"]
        percentile_scale=scale_config["percentile_scale"]
        percentile_cut=scale_config["percentile_cut"]
    else:
        sd_scale=0.9
        percentile_scale=0.9
        percentile_cut=0.5

    sdm_quantizer = SigmaDeltaQuantizer(first2=first2, 
                                        sd_scale=sd_scale, 
                                        order=1,
                                        percentile_scale=percentile_scale,
                                        percentile_cut=percentile_cut)#0.9 0.9 0.5原版
