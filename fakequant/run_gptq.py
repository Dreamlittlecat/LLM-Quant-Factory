...
from methods.gptq_utils.gptq import GPTQ
from methods.gptq_utils.quant import Quantizer
...
def quant_sequential(model, dataloader, dev):
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            gptq[name].quantizer.configure(bits=args.n_bits, perchannel=True, sym=False, mse=False, trits=False)
...
if __name__ == "__main__":
    ...
    save_title = f"{model_name}_{args.dataset}_{args.n_bits}_{groupsize}"
    save_file = "/data/xjh/model_weight/quant/gptq/" + save_title.replace("/", "_") 
